...
* The **data** This package includes two data sets (DNA sequences): samplemutseq.rda, and sampleseq.rda. 
...
We want to first read in the DNA sequence we want, the package depend on *seqinr* package for FASTA file reading. 
...
The package allows obtaining the mutation details by comparing the two DNA sequences we have just read in.
The *mutChecker()* function is used for the previous purpose. The function takes in two DNA sequence, the first is healthy version and the second is the suspected mutation one. 
...
# We print the table by summerizing the Matrix we have generated in the previous
# Section.
...
There are two plotting functions available, the *mutPlot* function takes in the mutation details generated by *mutChecker* function, and displays the number of mutations occuring along with the number of each base mutated. 
```{r mutPlot , message=FALSE}
mutPlot(mutvals = mutCheckerResults)
```

The *mutCompPlot* function visulize the number of bases in healthy and mutated sequence respectively, this gives a clearer view on how the two sequences differ. The function takes in two DNA sequence, the first is healthy version and the second is the suspected mutation one. 
